...
const CreateGroupModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  // ESTADOS DE IMAGEM
  const [imagePreview, setImagePreview] = useState(null); // Para mostrar na tela
  const [imageFile, setImageFile] = useState(null);       // Para enviar ao backend (Novo!)
  
  const [isPrivate, setIsPrivate] = useState(false);

  useEffect(() => {
    if (!isOpen) {
      // Limpa tudo ao fechar/abrir
  ...
  const handleGroupImageChange = (e) => {
    ...
    if (file) {
      // 1. Salva o arquivo real no estado para envio posterior
      setImageFile(file);

      // 2. Gera o preview para exibição imediata
  ...
  const handleSubmit = (e) => {
    ...
    const groupData = {
      ...
      // Passamos o arquivo real para o serviço usar no FormData
      imageFile: imageFile, 
      
      // Opcional: passamos o preview se quisermos atualização otimista na UI antes do reload
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="hidden"
                // Importante para resetar o input se o usuário selecionar o mesmo arquivo novamente após cancelar
